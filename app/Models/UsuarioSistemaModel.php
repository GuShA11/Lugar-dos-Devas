...
class UsuarioSistemaModel extends \Com\Daw2\Core\BaseModel {
    ...
    function insert(array $data): bool {
        $stmt = $this->pdo->prepare('INSERT INTO usuario_sistema(id_rol, user, pass, nombre, baja) values (?,?,?,?,?)');
        $pass = password_hash($data['pass'], PASSWORD_DEFAULT);
        return $stmt->execute([$data['id_rol'], $data['user'], $pass, $data['nombre'], 0]);
    }

    function update(array $data, int $id): bool {
        $stmt = $this->pdo->prepare('UPDATE usuario_sistema SET nombre=?, id_rol=?, user=? WHERE id_usuario=?');
        return $stmt->execute([$data['nombre'], $data['id_rol'], $data['user'], $id]);
    }
    ...
    function countByEmailNotUser(string $email, int $id_user): int {
        $stmt = $this->pdo->prepare('SELECT COUNT(*) FROM usuario_sistema WHERE user = ? AND id_usuario != ?');
